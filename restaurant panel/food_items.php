...
<body>
  ...
  <div class="container">
    <a href="dashboard.php" class="btn" style="background:#557174">← Back to Dashboard</a>
    <br><br>
      ...
      <form method="post" enctype="multipart/form-data">
        ...
        <select name="category_id" required>
          ...
          while ($c = $cats2->fetch_assoc()):
            ?>
            <option value="<?php echo $c['category_id']; ?>" <?php if ($edit && $edit['category_id'] == $c['category_id'])
                 echo 'selected'; ?>>
        ...
        <input class="input" type="text" name="name" placeholder="Food name" required
          value="<?php echo $edit ? htmlspecialchars($edit['name']) : ''; ?>">
        <input class="input" type="text" name="price" placeholder="Price" required
          value="<?php echo $edit ? htmlspecialchars($edit['price']) : ''; ?>">
        <br><br>
        ...
        <?php if ($edit && $edit['image']): ?>
          <div style="margin-top:8px"><img src="../assets/images/<?php echo htmlspecialchars($edit['image']); ?>"
              class="thumb"></div>
        <?php endif; ?>

        <input type="hidden" name="fid" value="<?php echo $edit ? intval($edit['food_id']) : 0; ?>">
        <div style="margin-top:8px"><button class="btn" name="save_food"
            type="submit"><?php echo $edit ? 'Update' : 'Save'; ?></button></div>
      </form>
    </div>
      ...
      <table class="table">
        ...
        <tbody>
          ...
          while ($it = $items->fetch_assoc()):
            ...
            <tr>
              <td><?php echo $i++; ?></td>
              <td><?php if ($it['image']): ?><img src="../assets/images/<?php echo htmlspecialchars($it['image']); ?>"
                    class="thumb"><?php endif; ?></td>
