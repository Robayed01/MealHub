    ...
    <div class="card">
      ...
      <form method="get" style="margin-top:12px" class="form-row">
        <label>Restaurant:
          <select name="restaurant_id" class="input">
            <option value="0" <?php if ($selected_rest === 0)
              echo ' selected'; ?>>-- All Restaurants --</option>
            <?php
            $resList = $conn->query("SELECT restaurant_id, name FROM restaurants ORDER BY name");
            while ($r = $resList->fetch_assoc()):
              ?>
              <option value="<?php echo (int) $r['restaurant_id']; ?>" <?php if ($selected_rest == (int) $r['restaurant_id'])
                    echo 'selected'; ?>>
        ...
        <label>Start: <input class="input" type="date" name="start_date"
            value="<?php echo htmlspecialchars($start); ?>"></label>
        <label>End: <input class="input" type="date" name="end_date"
            value="<?php echo htmlspecialchars($end); ?>"></label>
      ...
      <div style="display:flex;gap:20px;margin-top:18px">
        ...
        <div style="flex:1;padding:16px;background:#f8fff8;border-radius:8px;text-align:center">
          ...
          <div class="stat" style="color:green">Tk<?php echo number_format($revenue, 2); ?> </div>
        </div>
      </div>
    </div>
          ...
          <?php if ($daily_result && $daily_result->num_rows > 0): ?>
            <?php while ($r = $daily_result->fetch_assoc()): ?>
              <tr>
                ...
                <td>Tk<?php echo number_format((float) $r['revenue'], 2); ?></td>
              </tr>
            <?php endwhile; ?>
          <?php else: ?>
            <tr>
